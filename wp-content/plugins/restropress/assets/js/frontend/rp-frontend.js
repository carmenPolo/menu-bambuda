jQuery( function($) {
  ...
  $('.rpress-add-to-cart').click(function(e) {
    ...
    $.ajax({
      ...
      success : function( response ) {
        ...
        $( '#rpressModal .rpress-prices').html(response.data.price);
  ...
  $( '.rpress-sidebar-cart' ).on( 'click', 'a.rpress-edit-from-cart', function(e) {
    ...
    if( CartItemId !== '' ) {
      ...
      success: function(response) {
        ...
        $( '#rpressModal' ).find( '.submit-fooditem-button' ).text( rp_scripts.update_cart );
        $( '#rpressModal .modal-body' ).html( response.data.html );
      }
    });
    }
        ...
        $.ajax({
          ...
          success: function(response) {
            if( response ) {
              ...
              $('#rpressModal').css('display', 'none');

              self.text( rp_scripts.add_to_cart );
        ...
        $.ajax({
          ...
          success: function( response ) {
            ...
            if ( response ) {
              ...
              $('ul.rpress-cart').find('.cart-total').html(response.total);
              $('ul.rpress-cart').find('.subtotal').html(response.subtotal);
    ...
    $.ajax({
      ...
      success: function( response ) {
        ...
        if ( response.status == 'error' ) {
          ...
          rp_setCookie( 'service_type', serviceType, 1 );

          if ( serviceDate === undefined ) {

            rp_setCookie( 'service_date', rpress_scripts.current_date, 1 );
            rp_setCookie( 'delivery_date', rpress_scripts.display_date, 1 );

          } else {

            var delivery_date = $('.delivery-settings-wrapper.active .rpress_get_delivery_dates option:selected').text();
            rp_setCookie( 'service_date', serviceDate, 1 );
            rp_setCookie( 'delivery_date', delivery_date, 1 );
          }

          if( serviceTime === undefined ) {
            rp_setCookie( 'service_time', '', 1 );
          } else {
            rp_setCookie( 'service_time', serviceTime, 1 );
          }
          ...
          if ( foodItemId ) {
            ...
            setTimeout(function(){ $('#rpress_fooditem_'+foodItemId).find('.rpress-add-to-cart').trigger('click'); }, 600);
  ...
  //Update service type and service time
