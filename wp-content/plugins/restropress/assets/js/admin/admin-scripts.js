jQuery(document).ready(function ($) {
	...
	var RPRESS_RestroPress_Configuration = {
		...
		clone_repeatable : function(row) {
			...
			var key = highest = 1;
			row.parent().find( '.rpress_repeatable_row' ).each(function() {
			...
			clone.find( 'span.rpress_price_id' ).each(function() {
				$( this ).text( parseInt( key ) );
			});

			clone.find( 'span.rpress_file_id' ).each(function() {
				$( this ).text( parseInt( key ) );
			});

			clone.find( '.rpress_repeatable_default_input' ).each( function() {
				$( this ).val( parseInt( key ) ).removeAttr('checked');
			});

			clone.find( '.rpress_repeatable_condition_field' ).each ( function() {
		...
		add : function() {
			$( document.body ).on( 'click', '.submit .rpress_add_repeatable', function(e) {
				e.preventDefault();
				var button = $( this ),
				row = button.parent().parent().prev( '.rpress_repeatable_row' ),
				clone = RPRESS_RestroPress_Configuration.clone_repeatable(row);

				clone.insertAfter( row ).find('input, textarea, select').filter(':visible').eq(0).focus();

				// Setup chosen fields again if they exist
				clone.find('.rpress-select-chosen').chosen({
					inherit_select_classes: true,
					placeholder_text_single: rpress_vars.one_option,
					placeholder_text_multiple: rpress_vars.one_or_more_option,
				});
				clone.find( '.rpress-select-chosen' ).css( 'width', '100%' );
				clone.find( '.rpress-select-chosen .chosen-search input' ).attr( 'placeholder', rpress_vars.search_placeholder );
			});
		},

		move : function() {

			$(".rpress_repeatable_table .rpress-repeatables-wrap").sortable({
				handle: '.rpress-draghandle-anchor', items: '.rpress_repeatable_row', opacity: 0.6, cursor: 'move', axis: 'y', update: function() {
					var count  = 0;
					$(this).find( '.rpress_repeatable_row' ).each(function() {
						$(this).find( 'input.rpress_repeatable_index' ).each(function() {
							$( this ).val( count );
						});
						count++;
					});
		...
		remove : function() {
			$( document.body ).on( 'click', '.rpress-remove-row, .rpress_remove_repeatable', function(e) {
				e.preventDefault();

				var row   = $(this).parents( '.rpress_repeatable_row' ),
					count = row.parent().find( '.rpress_repeatable_row' ).length,
					type  = $(this).data('type'),
					...
					if ( $(this).is( '.ui-sortable .rpress_repeatable_row:first-child .rpress-remove-row, .ui-sortable .rpress_repeatable_row:first-child .rpress_remove_repeatable' ) ) {
				...
				if ( type === 'price' ) {
					...
					$( '.rpress_repeatable_condition_field option[value="' + price_row_id + '"]' ).remove();
				}
		...
		type : function() {
			...
			$( document.body ).on( 'change', '#_rpress_product_type', function(e) {
		...
		prices : function() {
			$( document.body ).on( 'change', '#rpress_variable_pricing', function(e) {
				var checked   = $(this).is(':checked');
		...
		files : function() {
			...
			$( document.body ).on('click', '.rpress_upload_file_button', function(e) {

				e.preventDefault();

				var button = $(this);

				window.formfield = $(this).closest('.rpress_repeatable_upload_wrapper');
				...
				file_frame.on( 'insert', function() {
					...
					selection.each( function( attachment, index ) {
						...
						if ( 0 === index ) {
							...
							window.formfield.find( '.rpress_repeatable_attachment_id_field' ).val( attachment.id );
							window.formfield.find( '.rpress_repeatable_thumbnail_size_field').val( selectedSize );
							window.formfield.find( '.rpress_repeatable_upload_field' ).val( selectedURL );
							window.formfield.find( '.rpress_repeatable_name_field' ).val( selectedName );
						} else {
							// Create a new row for all additional attachments
							var row = window.formfield,
								clone = RPRESS_RestroPress_Configuration.clone_repeatable( row );

							clone.find( '.rpress_repeatable_attachment_id_field' ).val( attachment.id );
							clone.find( '.rpress_repeatable_thumbnail_size_field' ).val( selectedSize );
							clone.find( '.rpress_repeatable_upload_field' ).val( selectedURL );
							clone.find( '.rpress_repeatable_name_field' ).val( selectedName );
		...
		updatePrices: function() {
			$( '#rpress_price_fields' ).on( 'keyup', '.rpress_variable_prices_name', function() {

				var key = $( this ).parents( '.rpress_repeatable_row' ).data( 'key' ),
					name = $( this ).val(),
				...
				if ( field_option.length > 0 ) {
					...
					$( '.rpress_repeatable_condition_field' ).append(
						$( '<option></option>' )
							.attr( 'value', key )
							.text( name )
					);
	...
	$( document.body ).on( 'click', '.toggle-custom-price-option-section', function(e) {
		e.preventDefault();
		var show = $(this).html() == rpress_vars.show_advanced_settings ? true : false;

		if ( show ) {
			$(this).html( rpress_vars.hide_advanced_settings );
		} else {
			$(this).html( rpress_vars.show_advanced_settings );
		}

		var header = $(this).parents('.rpress-repeatable-row-header');
		header.siblings('.rpress-custom-price-option-sections-wrap').slideToggle();
	...
	var RPRESS_Edit_Payment = {
		...
		edit_address : function() {
			...
			$('select[name="rpress-payment-address[0][country]"]').change(function() {
				var $this = $(this);
				var data = {
				...
				$.post(ajaxurl, data, function (response) {
					...
					$( '#rpress-order-address-state-wrap .chosen-container' ).remove();
		...
		remove_fooditem : function() {
			...
			$( '#rpress-purchased-items' ).on('click', '.rpress-order-remove-fooditem', function() {

				var count = $( document.body ).find( '#rpress-purchased-items > .row:not(.header)' ).length;
				...
				if( confirm( rpress_vars.delete_payment_fooditem ) ) {
					...
					var purchase_id = $('.rpress-payment-id').val();
					var fooditem_id = $('input[name="rpress-payment-details-fooditems['+key+'][id]"]').val();
					var price_id    = $('input[name="rpress-payment-details-fooditems['+key+'][price_id]"]').val();
					var quantity    = $('input[name="rpress-payment-details-fooditems['+key+'][quantity]"]').val();
					var amount      = $('input[name="rpress-payment-details-fooditems['+key+'][amount]"]').val();

					if ( $('input[name="rpress-payment-details-fooditems['+key+'][tax]"]') ) {
						var fees = $('input[name="rpress-payment-details-fooditems['+key+'][tax]"]').val();
					}
					...
					var currently_removed  = $('input[name="rpress-payment-removed"]').val();
					currently_removed      = $.parseJSON(currently_removed);
					if ( currently_removed.length < 1 ) {
						currently_removed  = {};
					}

					var removed_item       = [ { 'id': fooditem_id, 'price_id': price_id, 'quantity': quantity, 'amount': amount, 'cart_index': key } ];
					currently_removed[key] = removed_item

					$('input[name="rpress-payment-removed"]').val(JSON.stringify(currently_removed));

					$(this).parents('.row.rpress-purchased-row').remove();
					if ( fees && fees.length) {
		...
		change_customer : function() {
			...
			$('#rpress-customer-details').on('click', '.rpress-payment-change-customer, .rpress-payment-change-customer-cancel', function(e) {
				e.preventDefault();

				var change_customer = $(this).hasClass('rpress-payment-change-customer');
				var cancel          = $(this).hasClass('rpress-payment-change-customer-cancel');

				if ( change_customer ) {
					$('.customer-info').hide();
					$('.change-customer').show();
					$('.rpress-payment-change-customer-input').css('width', 'auto');
				} else if( cancel) {
					$('.customer-info').show();
		...
		new_customer : function() {
			...
			$('#rpress-customer-details').on('click', '.rpress-payment-new-customer, .rpress-payment-new-customer-cancel', function(e) {
				e.preventDefault();

				var new_customer = $(this).hasClass('rpress-payment-new-customer');
				var cancel       = $(this).hasClass('rpress-payment-new-customer-cancel');
		...
		add_fooditem : function() {
			...
			$('.rpress-edit-purchase-element').on('click', '#rpress-order-add-fooditem', function(e) {
				...
				clone.find( '.fooditem span.rpress-purchased-fooditem-title' ).html( '<a href="post.php?post=' + fooditem_id + '&action=edit"></a>' );
		...
		edit_qty: function() {
			$(document.body).on('change keyup', '.rpress-payment-details-fooditem-quantity', function() {
				var selectedQty = $(this).val();
				var row = $(this).parents('ul.rpress-purchased-items-list-wrapper');

				row.find('input.rpress-payment-details-fooditem-quantity').val(selectedQty);
			});
		},
		...
		edit_price : function() {
			$(document.body).on('change keyup', '.rpress-payment-item-input', function () {
				...
				var quantity   = row.find('input.rpress-payment-details-fooditem-quantity').val().replace(rpress_vars.thousands_separator,'');
				var item_price = row.find('input.rpress-payment-details-fooditem-item-price').val().replace(rpress_vars.thousands_separator,'');
				var item_tax   = row.find('input.rpress-payment-details-fooditem-item-tax').val().replace(rpress_vars.thousands_separator,'');
				...
				$(".addon-items-list").each(function(key, item) {
					...
					if( addonTotalPrice !== null ) {
						for (var i = 0; i < addonTotalPrice.length; i++) {
							...
							addonData = addonData[2];
    					addonTotal += parseFloat(addonData);
						}
					}
		...
		variable_prices_check : function() {
			...
			$('.rpress-edit-purchase-element').on('change', 'select#rpress_order_fooditem_select', function() {

				var $this = $(this), fooditem_id = $this.val();
		...
		add_note : function() {
			...
			$('#rpress-add-payment-note').on('click', function(e) {
				e.preventDefault();
				var postData = {
					action : 'rpress_insert_payment_note',
					payment_id : $(this).data('payment-id'),
					note : $('#rpress-payment-note').val()
				};
				...
				if( postData.note ) {
					...
					$.ajax({
						...
						url: ajaxurl,
						success: function (response) {
							$('#rpress-payment-notes-inner').append( response );
							$('.rpress-no-payment-notes').hide();
							$('#rpress-payment-note').val('');
						}
					}).fail(function (data) {
						if ( window.console && window.console.log ) {
							console.log( data );
					...
					var border_color = $('#rpress-payment-note').css('border-color');
					$('#rpress-payment-note').css('border-color', 'red');
					setTimeout( function() {
						$('#rpress-payment-note').css('border-color', border_color );
					}, 500 );
				}
		...
		remove_note : function() {
			...
			$( document.body ).on('click', '.rpress-delete-payment-note', function(e) {
				...
				if( confirm( rpress_vars.delete_payment_note) ) {
					...
					$.ajax({
						...
						url: ajaxurl,
						success: function (response) {
							$('#rpress-payment-note-' + postData.note_id ).remove();
							if( ! $('.rpress-payment-note').length ) {
								$('.rpress-no-payment-notes').show();
							}
							return false;
						}
					}).fail(function (data) {
		...
		resend_receipt : function() {
			...
			$( document.body ).on( 'click', '#rpress-select-receipt-email', function( e ) {
			...
			$( document.body ).on( 'change', '.rpress-order-resend-receipt-email', function() {

				var href = $('#rpress-select-receipt-email').prop( 'href' ) + '&email=' + $(this).val();
			...
			$( document.body ).on( 'click', '#rpress-resend-receipt', function( e ) {
		...
		copy_fooditem_link : function() {
			$( document.body ).on( 'click', '.rpress-copy-fooditem-link', function( e ) {
				e.preventDefault();
				var $this    = $(this);
				var postData = {
					action      : 'rpress_get_file_fooditem_link',
					payment_id  : $('input[name="rpress_payment_id"]').val(),
					fooditem_id : $this.data('fooditem-id'),
				...
				$.ajax({
					...
					success: function (link) {
						$( "#rpress-fooditem-link" ).dialog({
							width: 400
						}).html( '<textarea rows="10" cols="40" id="rpress-fooditem-link-textarea">' + link + '</textarea>' );
						$( "#rpress-fooditem-link-textarea" ).focus().select();
						return false;
					}
				}).fail(function (data) {
					if ( window.console && window.console.log ) {
	...
	var RPRESS_Discount = {
		...
		type_select : function() {
			...
			$('#rpress-edit-discount #rpress-type, #rpress-add-discount #rpress-type').change(function() {
				var val = $(this).val();
				$('.rpress-amount-description').hide();
				$('.rpress-amount-description.' + val + '-discount').show();
	...
	var RPRESS_Reports = {
		...
		date_options : function() {
			...
			$( '#rpress-graphs-date-options' ).change( function() {
				var $this = $(this),
		...
		customers_export : function() {
			...
			$( '#rpress_customer_export_fooditem' ).change( function() {

				var $this = $(this),
					fooditem_id = $('option:selected', $this).val(),
				...
				if ( parseInt( fooditem_id ) != 0 ) {
					...
					$.post(ajaxurl, data, function(response) {
						...
						$('#rpress_customer_export_fooditem_chosen').after( response );
					});
				} else {
					price_options_select.remove();
				}
	...
	var RPRESS_Settings = {
		...
		general : function() {
			...
			if ( typeof wp === "undefined" || '1' !== rpress_vars.new_media_ui ) {
				...
				if ( rpress_settings_upload_button.length > 0 ) {
					...
					$( document.body ).on('click', rpress_settings_upload_button, function(e) {
						e.preventDefault();
						window.formfield = $(this).parent().prev();
						window.tbframe_interval = setInterval(function() {
							jQuery('#TB_iframeContent').contents().find('.savesend .button').val(rpress_vars.use_this_file).end().find('#insert-gallery, .wp-post-thumbnail').hide();
						}, 2000);
		...
		taxes : function() {
			...
			// Update base state field based on selected base country
			$('select[name="rpress_settings[base_country]"]').change(function() {
			...
			$( document.body ).on('change', '#rpress_tax_rates select.rpress-tax-country', function() {
				var $this = $(this);
				var data = {
					action: 'rpress_get_states',
					country: $(this).val(),
			...
			$('#rpress_add_tax_rate').on('click', function() {
			...
			// Remove tax row
			$( document.body ).on('click', '#rpress_tax_rates .rpress_remove_tax_rate', function() {
				if( confirm( rpress_vars.delete_tax_rate ) ) {
					var tax_rates = $('#rpress_tax_rates tr:visible');
					var count     = tax_rates.length;

					if( count === 2 ) {
						$('#rpress_tax_rates select').val('');
						$('#rpress_tax_rates input[type="text"]').val('');
						$('#rpress_tax_rates input[type="number"]').val('');
						$('#rpress_tax_rates input[type="checkbox"]').attr('checked', false);
					} else {
						$(this).closest('tr').remove();
					}

					/* re-index after deleting */
					$('#rpress_tax_rates tr').each( function( rowIndex ) {
		...
		misc : function() {
			...
			var symlink = fooditemMethod.parent().parent().next();
	...
	$('.fooditem_page_rpress-payment-history .row-actions .delete a, a.rpress-delete-payment').on('click', function() {
	...
	$('body').on('click', '#the-list .editinline', function() {
		...
		var regprice = $rpress_inline_data.find('.column-price .fooditemprice-' + post_id).val();

		// If variable priced product disable editing, otherwise allow price changes
		if ( regprice != $('#post-' + post_id + '.column-price .fooditemprice-' + post_id).val() ) {
			$('.regprice', '#rpress-fooditem-data').val(regprice).attr('disabled', false);
		} else {
			$('.regprice', '#rpress-fooditem-data').val( rpress_vars.quick_edit_warning ).attr('disabled', 'disabled');
		}
	});
	...
	$( document.body ).on( 'click', '#bulk_edit', function() {
		...
		var $price = $( '#rpress-fooditem-data input[name="_rpress_regprice"]' ).val();
	...
	// Setup Chosen menus
	$('.rpress-select-chosen').chosen({
		inherit_select_classes: true,
		placeholder_text_single: rpress_vars.one_option,
		placeholder_text_multiple: rpress_vars.one_or_more_option,
	});

	$('.rpress-select-chosen .chosen-search input').each( function() {
		var selectElem = $(this).parent().parent().parent().prev('select.rpress-select-chosen'),
	...
	$( document.body ).on( 'keyup', '.rpress-select.chosen-container .chosen-search input, .rpress-select.chosen-container .search-field input', function(e) {

		var val         = $(this).val()
		var container   = $(this).closest( '.rpress-select-chosen' );
	...
	$( '#post' ).on( 'click', '.rpress-thickbox', function() {
		$( '.rpress-select-chosen', '#choose-fooditem' ).css( 'width', '100%' );
	});
	...
	var RPRESS_Tools = {
		...
		revoke_api_key : function() {
			$( document.body ).on( 'click', '.rpress-revoke-api-key', function( e ) {
				return confirm( rpress_vars.revoke_api_key );
			} );
		},
		regenerate_api_key : function() {
			$( document.body ).on( 'click', '.rpress-regenerate-api-key', function( e ) {
		...
		recount_stats : function() {
			$( document.body).on( 'change', '#recount-stats-type', function() {
				...
				products.hide();
				$('.rpress-recount-stats-descriptions span').hide();

				if ( 'recount-fooditem' === selected_type ) {

					products.show();
					products.find('.rpress-select-chosen').css('width', 'auto');
			...
			$( '#rpress-tools-recount-form' ).submit( function(e) {
				...
				if ( 'recount-fooditem' === selected_type ) {
					...
					var selected_fooditem = $('select[name="fooditem_id"]').val();
	...
	var RPRESS_Export = {
		...
		submit : function() {
			...
			$( document.body ).on( 'submit', '.rpress-export-form', function(e) {
				e.preventDefault();
				...
				if ( ! submitButton.hasClass( 'button-disabled' ) ) {
					...
					$(this).append( '<div class="notice-wrap"><span class="spinner is-active"></span><div class="rpress-progress"><div></div></div></div>' );
			...
			$.ajax({
				...
				success: function( response ) {
					if( 'done' == response.step || response.error || response.success ) {
						...
						var export_form    = $('.rpress-export-form').find('.rpress-progress').parent().parent();
						var notice_wrap    = export_form.find('.notice-wrap');
		...
		dismiss_message : function() {
			$(document.body).on( 'click', '#rpress-batch-success .notice-dismiss', function() {
				$('#rpress-batch-success').parent().slideUp('fast');
			});
		}
	...
	var RPRESS_Import = {
		...
		before_submit : function( arr, $form, options ) {
			...
			if ( window.File && window.FileReader && window.FileList && window.Blob ) {
				...
				var import_form = $('.rpress-import-form').find('.rpress-progress').parent().parent();
		...
		complete: function( xhr ) {
			...
			if( response.success ) {
				...
				var $form = $('.rpress-import-form .notice-wrap').parent();

				$form.find('.rpress-import-file-wrap,.notice-wrap').remove();

				$form.find('.rpress-import-options').slideDown();
				...
				$(document.body).on('click', '.rpress-import-proceed', function(e) {
				...
				success: function( response ) {
					...
					if( 'done' == response.data.step || response.data.error ) {
						...
						if ( response.data.error ) {
							...
							import_form.find( '.rpress-import-options' ).hide();
							$('html, body').animate({
	...
	var RPRESS_Customer = {
		...
		edit_customer: function() {
			$( document.body ).on( 'click', '#edit-customer', function( e ) {
				...
				RPRESS_Customer.vars.customer_card_wrap_edit_item.fadeIn().css( 'display', 'block' );
			});
		},
		add_email: function() {
		...
		user_search: function() {
			...
			$( document.body ).on('click.rpressSelectUser', '.rpress_user_search_results a', function( e ) {
				e.preventDefault();
			...
			$( document.body ).on( 'click', '#add-customer-note', function( e ) {
				...
				if( postData.customer_note ) {
					...
					$.ajax({
						...
						url: ajaxurl,
						success: function ( response ) {
							$( '#rpress-customer-notes' ).prepend( response );
							$( '.rpress-no-customer-notes' ).hide();
		...
		delete_checked: function() {
			$( '#rpress-customer-delete-confirm' ).change( function() {
	...
	// AJAX user search
	$('.rpress-ajax-user-search').keyup(function() {
		...
		$.ajax({
			...
			success: function (search_response) {
				...
				$('.rpress-ajax').hide();
				$('.rpress_user_search_results').removeClass('hidden');
				$('.rpress_user_search_results span').html('');
				$(search_response.results).appendTo('.rpress_user_search_results span');
				document.body.style.cursor = 'default';
			}
		});
	});

	$( document.body ).on('click.rpressSelectUser', '.rpress_user_search_results span a', function(e) {
		e.preventDefault();
		var login = $(this).data('login');
		$('.rpress-ajax-user-search').val(login);
		$('.rpress_user_search_results').addClass('hidden');
		$('.rpress_user_search_results span').html('');
	});

	$( document.body ).on('click.rpressCancelUserSearch', '.rpress_user_search_results a.rpress-ajax-user-cancel', function(e) {
		e.preventDefault();
		$('.rpress-ajax-user-search').val('');
		$('.rpress_user_search_results').addClass('hidden');
		$('.rpress_user_search_results span').html('');
	});

	if( $('#rpress_dashboard_sales').length ) {
		$.ajax({
			type: "GET",
			data: {
			...
			url: ajaxurl,
			success: function (response) {
				$('#rpress_dashboard_sales .inside').html( response );
			}
		});
	}
...
var rpressLegendFormatterSales = function (label, series) {
	...
	jQuery('#rpress-pie-legend-' + series.rpress_vars.id).append( item );
	return item;
}
...
jQuery(document).on( 'change', '.rpress-get-variable-prices input[type=radio]', function() {
  var selectedPrice = jQuery(this).val();
  jQuery(this).parents('.rpress-get-variable-prices').find('.rpress_selected_price').val(selectedPrice);
});
