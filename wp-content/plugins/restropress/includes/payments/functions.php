...
function rpress_insert_payment( $payment_data = array() ) {
	...
	$payment->delivery_type      = isset($_COOKIE['service_type']) ? $_COOKIE['service_type'] : ''  ;
	$payment->delivery_time      = isset($_COOKIE['service_time']) ? $_COOKIE['service_time'] : ''  ;
	$payment->delivery_date     = isset($_COOKIE['delivery_date']) ? $_COOKIE['delivery_date'] : ''  ;
...
function rpress_get_payment_statuses() {
	$payment_statuses = array(
		'pending' 		=> __( 'Pending Payment', 'restropress' ),
...
function rpress_get_discount_price_by_payment_id( $payment_id = 0 ) {
    ...
    if ( is_array( $cart_contents ) && !empty( $cart_contents ) ) {
        foreach( $cart_contents as $key => $cart_content ) {
            $discount = isset( $cart_content['discount'] ) ? $cart_content['discount'] : '0.00';
        }
    }
