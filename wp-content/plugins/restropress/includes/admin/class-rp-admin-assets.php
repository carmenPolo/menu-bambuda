...
if ( ! class_exists( 'RP_Admin_Assets', false ) ) :
  ...
  class RP_Admin_Assets {
    ...
    public function admin_styles() {
      ...
      wp_register_style( 'timepicker', plugins_url( 'assets/css/jquery.timepicker.css', RP_PLUGIN_FILE ), array(), RP_VERSION );
      ...
      wp_register_style( 'jquery-chosen', plugins_url( 'assets/css/chosen.min.css', RP_PLUGIN_FILE ), array(), RP_VERSION );

      $ui_style = ( 'classic' == get_user_option( 'admin_color' ) ) ? 'classic' : 'fresh';
      wp_register_style( 'jquery-ui-css', plugins_url( 'assets/css/jquery-ui-'. $ui_style . '.min.css', RP_PLUGIN_FILE ) );
    ...
    public function admin_scripts() {
      ...
      wp_register_script( 'rp-admin-meta-boxes', RP_PLUGIN_URL . 'assets/js/admin/meta-boxes' . $suffix . '.js', array( 'jquery', 'jquery-ui-datepicker', 'jquery-ui-sortable', 'select2', 'jquery-tiptip', 'jquery-blockui' ), RP_VERSION );

      wp_register_script( 'rpress-orders', RP_PLUGIN_URL . 'assets/js/admin/rp-orders' . $suffix . '.js', array( 'jquery', 'rp-backbone-modal' ), RP_VERSION );

      wp_register_script( 'jquery-toast', RP_PLUGIN_URL . 'assets/js/admin/jquery.toast.js', array( 'jquery' ), RP_VERSION );

      wp_register_script( 'rp-admin', RP_PLUGIN_URL . 'assets/js/admin/rp-admin.js', array( 'jquery', 'jquery-toast', 'timepicker' ), RP_VERSION );
      ...
      $admin_deps = array( 'jquery', 'jquery-form', 'inline-edit-post', 'jquery-ui-tooltip' );

      wp_register_script( 'rpress-admin-scripts', RP_PLUGIN_URL . 'assets/js/admin/admin-scripts' . $suffix . '.js', $admin_deps, RP_VERSION, false );
      ...
      $admin_params = array(
        ...
        'please_wait'     => esc_html( 'Please Wait', 'restropress' ),
        ...
        'deactivate_license'  => esc_html( 'Deactivate License', 'restropress' ),
        ...
        'type_to_search'              => sprintf( __( 'Type to search %s', 'restropress' ), rp_get_label_plural() ), /* translators: %s: singular fooditem */
      ...
      wp_register_script( 'rpress-admin-scripts-compatibility', RP_PLUGIN_URL . '/assets/js/admin/admin-backwards-compatibility' . $suffix . '.js', array( 'jquery', 'rpress-admin-scripts' ), RP_VERSION );
      ...
      if ( in_array( $screen_id, array( 'fooditem', 'edit-fooditem' ) ) ){
        wp_register_script( 'rp-admin-fooditem-meta-boxes', RP_PLUGIN_URL . 'assets/js/admin/meta-boxes-fooditem' . $suffix . '.js', array( 'rp-admin-meta-boxes' ), RP_VERSION );
